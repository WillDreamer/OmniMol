...
from model.configs import PureTextConfig
...
class TextPreTrainedModel(PreTrainedModel):
    config_class = PureTextConfig
...
class TextModelForConditionalGeneration(TextPreTrainedModel, GenerationMixin):
    def __init__(self, config: PureTextConfig):
    ...
    def remove_multimodal(
        ...
        fused_embeds = torch.zeros(
            size=[input_ids.shape[0], input_ids.shape[1], self.config.text_config.hidden_size],
            dtype=dtype,
            device=device
        )
        ...
        for batch_idx in range(input_ids.shape[0]):
            ...
            fused_embeds[batch_idx, mm_pos:mm_pos+len(right_side_ids), :] = right_side_text_embedding
            # d. fill into labels
            if labels is not None:
                ...
                fused_labels[batch_idx, mm_pos:mm_pos+len(right_side_labels)] = right_side_labels
            ...
            fused_attention_mask[batch_idx, mm_pos:mm_pos+len(right_side_mask)] = right_side_mask
    ...
    def forward(
        ...
        **kwargs
        ...
        inputs_embeds, labels, input_ids, attention_mask, past_key_values, position_ids = self.remove_multimodal(
